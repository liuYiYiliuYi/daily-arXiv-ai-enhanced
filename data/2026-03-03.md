...
TL;DR: Proposes a discriminative segmentation model based on Qwen3-0.6B with cross-window context fusion for long-document topic segmentation, achieving better F1 scores and 100x faster inference than generative models.


<details>
  <summary>Details</summary>
Motivation: Existing methods have limitations in ultra-long text settings: traditional discriminative models lack document-level semantics due to fixed windows, while generative LLMs are computationally expensive and struggle with long inputs.

Method: Develops a discriminative segmentation model using Qwen3-0.6B backbone with added cross-window context fusion layer and boundary classification head, combined with overlapping sliding-window strategy. Also introduces vector fusion method with scalar correction for compressing ultra-long segment representations.

Result: Model supports 13k token inputs and can be extended to ultra-long documents. On WIKI-727K dataset, outperforms three Qwen2-0.5B generative models in macro-averaged F1 while providing two orders of magnitude faster inference.

Conclusion: The proposed discriminative approach significantly improves practicality and scalability of long-document processing by balancing accuracy with computational efficiency, making ultra-long document topic segmentation more feasible for real-world applications.
...
TL;DR: Task-Lens is a cross-task survey that evaluates 50 Indian speech datasets across 26 languages for 9 downstream speech tasks, identifying untapped metadata potential and underserved areas.
...
Method: Proposed Task-Lens, a cross-task survey methodology that: 1) Analyzes which of 50 Indian speech datasets contain metadata and properties suitable for 9 specific downstream speech tasks, 2) Proposes task-aligned enhancements to unlock datasets' full downstream potential, and 3) Identifies critically underserved tasks and Indian languages.
...
Conclusion: Cross-task profiling through Task-Lens addresses data scarcity in Indian speech technologies by revealing hidden potential in existing datasets and identifying critical gaps, facilitating more efficient resource allocation and research prioritization in linguistically diverse regions.
...
TL;DR: SLATE reduces credit assignment problems in LLM search reasoning via truncated step-level sampling and dense LLM-as-judge rewards, achieving lower variance and better performance than sparse or heuristic reward methods.


<details>
  <summary>Details</summary>
Motivation: Existing methods for training LLMs to reason with search engines suffer from credit assignment problems: sparse outcome rewards make it hard to attribute success/failure to individual decisions, while process-reward methods rely on heuristic rewards and still have high gradient variance.

Method: SLATE combines two key techniques: (1) truncated step-level sampling that generates trajectories sharing a common prefix but differing at the next step, and (2) dense LLM-as-judge rewards that replace heuristic scoring with capable LLM evaluators to assess reasoning steps, search queries, and answers.

Result: Theoretically proves truncated sampling reduces variance of advantage estimates by up to factor T for T-step trajectories. Experiments on seven QA benchmarks show SLATE consistently outperforms sparse-reward and process-reward baselines, with largest gains on harder multi-hop tasks and smaller models.

Conclusion: SLATE effectively addresses credit assignment in search-augmented LLM training through truncated sampling and dense LLM-judged rewards, providing lower-variance policy gradients and superior performance across diverse QA tasks.
...
TL;DR: First comprehensive benchmark and detection framework for hallucinated citations in scientific writing, using multi-agent verification to assess whether cited sources truly support claims, outperforming prior methods.


<details>
  <summary>Details</summary>
Motivation: Large language models introduce risk of fabricated references that appear plausible but correspond to no real publications; hallucinated citations have been observed in submissions at major ML venues, exposing peer review vulnerabilities; manual verification is impractical due to growing reference lists; existing automated tools are fragile to noisy citation formats and lack standardized evaluation.

Method: Multi-agent verification pipeline decomposes citation checking into claim extraction, evidence retrieval, passage matching, reasoning, and calibrated judgment; constructs large-scale human-validated dataset across domains; defines unified metrics for citation faithfulness and evidence alignment.
...
TL;DR: FHIRPath-QA is the first open dataset and benchmark for patient-specific clinical question answering using FHIRPath queries over real-world EHR data, enabling text-to-FHIRPath synthesis to reduce LLM usage and improve safety.


<details>
  <summary>Details</summary>
Motivation: Current EHR interfaces don't support precise, trustworthy answers to patient-specific questions, and retrieval-based LLM approaches are inefficient, prone to hallucination, and difficult to deploy over real EHRs.

Method: Proposed text-to-FHIRPath QA paradigm that shifts reasoning from free-text generation to FHIRPath query synthesis. Built dataset on MIMIC-IV on FHIR Demo with over 14k natural language questions paired with validated FHIRPath queries and answers.

Result: State-of-the-art LLMs struggle with ambiguity in patient language and perform poorly in FHIRPath query synthesis, but benefit strongly from supervised fine-tuning. Text-to-FHIRPath synthesis shows potential for safe, efficient, interoperable consumer health applications.

Conclusion: FHIRPath-QA dataset and benchmark provide a foundation for future research on text-to-FHIRPath synthesis, which could enable practical, safe, and efficient consumer health applications by reducing LLM usage and improving reliability.
...
TL;DR: IDP Accelerator is an agentic AI framework for end-to-end document intelligence that handles multi-document packets, complex reasoning, and compliance requirements through four key components: document segmentation, multimodal extraction, agentic analytics, and rule validation.


<details>
  <summary>Details</summary>
Motivation: Traditional document processing pipelines fail to handle multi-document packets, complex reasoning, and strict compliance requirements in industrial NLP settings, despite LLMs enabling zero-shot extraction capabilities.

Method: Four-component framework: (1) DocSplit benchmark dataset with multimodal BIO tagging classifier for document segmentation; (2) configurable Extraction Module using multimodal LLMs; (3) Agentic Analytics Module compliant with Model Context Protocol for secure data access; (4) Rule Validation Module using LLM-driven logic for compliance checks.
...
Conclusion: IDP Accelerator provides an effective open-source framework for industrial document intelligence that addresses limitations of traditional pipelines through agentic AI and multimodal LLM integration, with demonstrated real-world performance improvements.
...
TL;DR: ProbCOPA dataset reveals LLMs fail to match human probabilistic reasoning patterns on open-ended inferences, showing persistent differences in non-deterministic reasoning.


<details>
  <summary>Details</summary>
Motivation: While LLMs excel at logical/mathematical tasks, their performance on open-ended probabilistic inferences (common in human reasoning) remains unexplored, creating a gap in understanding how models handle non-deterministic reasoning.

Method: Created ProbCOPA dataset of 210 handcrafted probabilistic inferences in English, each annotated by 25-30 human participants for inference likelihood. Compared human judgments with responses from 8 state-of-the-art reasoning LLMs and analyzed LLM reasoning chains.

Result: Human responses showed graded, varied probabilistic judgments. LLMs consistently failed to produce human-like distributions. Analysis revealed common reasoning patterns in LLMs for evaluating such inferences, highlighting persistent human-LLM differences.

Conclusion: LLMs demonstrate significant limitations in probabilistic reasoning compared to humans, underscoring the need to evaluate reasoning beyond deterministic settings and revealing fundamental differences in how humans and models handle uncertainty.
...
TL;DR: The paper analyzes how seemingly redundant disjunctions become acceptable in specific contexts, providing a neural mechanism explanation to complement existing symbolic analyses.


<details>
  <summary>Details</summary>
Motivation: To understand why formally redundant sentences like "She will go to France or Spain, or perhaps to Germany or France" become acceptable in specific contexts, and to provide a neural mechanism explanation that complements existing symbolic formal analyses.

Method: 1) Present new behavioral evidence from humans and large language models demonstrating robustness of apparent non-redundancy across contexts. 2) Analyze language models to show redundancy avoidance arises from two interacting mechanisms: binding contextually relevant information to repeated lexical items, and Transformer induction heads selectively attending to these context-licensed representations.

Result: Demonstrated that apparent redundancy becomes acceptable through contextual licensing, and identified specific neural mechanisms in language models that enable this context-sensitive interpretation: binding contextual information to repeated items and selective attention by induction heads.

Conclusion: The neural explanation sheds light on mechanisms underlying context-sensitive semantic interpretation and complements existing symbolic analyses, showing how artificial neural networks can provide insights into human language processing phenomena.
...
TL;DR: MACR framework uses multi-agent causal reasoning with counterfactual user reactions and bias mitigation for suicide risk detection in online conversations.


<details>
  <summary>Details</summary>
Motivation: Existing suicide risk detection methods on social media have limitations: 1) they rely on predefined rules that capture only narrow user interactions, and 2) they overlook hidden influences like user conformity and suicide copycat behavior that affect suicidal expression propagation.

Method: Proposes Multi-Agent Causal Reasoning (MACR) framework with two collaborative agents: 1) Reasoning Agent integrates cognitive appraisal theory to generate counterfactual user reactions to posts, analyzing them through cognitive, emotional, and behavioral patterns with dedicated sub-agents; 2) Bias-aware Decision-Making Agent mitigates hidden biases through front-door adjustment strategy using counterfactual reactions.

Result: Extensive experiments on real-world conversational datasets demonstrate MACR's effectiveness and robustness in identifying suicide risk.

Conclusion: MACR framework addresses limitations of existing approaches by scaling user interactions through counterfactual reasoning and mitigating harmful biases from hidden influences, improving suicide risk detection in online communities.
...
TL;DR: BRIDGE framework reduces bias in automated essay scoring by generating synthetic high-scoring ELL samples through content transplantation from non-ELL essays while preserving authentic ELL linguistic patterns.


<details>
  <summary>Details</summary>
Motivation: Automated scoring systems using deep learning/LLMs amplify bias against underrepresented groups like English Language Learners (ELLs), particularly due to representation bias where scarce high-scoring ELL samples cause models to favor majority linguistic patterns, leading to unfair under-prediction of ELL students with comparable domain knowledge.

Method: BRIDGE (Bias-Reducing Inter-group Data GEneration) synthesizes high-scoring ELL samples by extracting construct-relevant content (rubric-aligned knowledge/evidence) from abundant high-scoring non-ELL samples and "pasting" it into authentic ELL linguistic patterns, with a discriminator model ensuring synthetic sample quality.
...
Conclusion: BRIDGE offers a cost-effective solution for equitable automated scoring in low-resource assessment settings by addressing representation bias through synthetic data generation rather than requiring additional real minority samples.
...
TL;DR: LFQA-HP-1M: A large-scale dataset of 1.3M human pairwise preferences for long-form QA evaluation, with rubric-based linear models matching LLM evaluators while exposing their biases.


<details>
  <summary>Details</summary>
Motivation: Existing metrics for long-form question answering (LFQA) often fail to align with human judgment, creating a need for better evaluation methods and datasets that capture nuanced multi-sentence explanatory responses.

Method: Created LFQA-HP-1M dataset with 1.3M human pairwise preference annotations; proposed nine rubrics for answer quality evaluation; developed simple linear models based on these features; examined transitivity consistency, positional bias, verbosity biases, and adversarial vulnerability in LLM evaluators.

Result: Rubric-based linear models performed comparably to state-of-the-art LLM evaluators; identified significant biases in LLM evaluators including transitivity inconsistencies, positional bias, verbosity bias, and vulnerability to adversarial perturbations.

Conclusion: Provides one of the largest public LFQA preference datasets and establishes a rubric-driven framework for transparent and reliable evaluation, offering a simpler alternative to complex LLM-based evaluators while exposing their limitations.
...
TL;DR: Proposes LLM-driven framework for synthesizing multi-turn task-oriented dialogues with realistic reasoning scenarios using trilevel optimization, creating benchmark to evaluate LLMs' practical reasoning capabilities.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks insufficiently reflect real-world complexity for evaluating LLM reasoning; current datasets are simplistic, disconnected from realistic task flows, domain constraints, and operational rules; data contamination undermines evaluation reliability; traditional crowdsourcing methods are labor-intensive and difficult to scale.

Method: LLM-driven framework for synthesizing multi-turn, task-oriented dialogues grounded in realistic reasoning scenarios using trilevel optimization; generates dialogues with authentic task scenarios enriched with real-world information and strong contextual coherence; designs corresponding reasoning tasks around dialogues with iterative refinement.

Result: Synthetic data-based reasoning tasks introduce non-trivial reasoning challenges and provide meaningful support for improving LLMs' reasoning capabilities; resulting dataset serves as valuable benchmark for assessing and advancing realistic logical reasoning capabilities.

Conclusion: Proposed framework addresses limitations of existing benchmarks by generating realistic, multi-turn task-oriented dialogues that better evaluate LLM reasoning in practical contexts, offering scalable solution to dataset construction challenges.
...
TL;DR: TRIZ-RAGNER: A retrieval-augmented LLM framework for TRIZ-aware named entity recognition in patent contradiction mining, achieving 84.2% F1-score by integrating TRIZ knowledge retrieval with structured LLM prompting.


<details>
  <summary>Details</summary>
Motivation: Existing TRIZ contradiction mining approaches rely on rule-based systems or traditional ML models that struggle with semantic ambiguity, domain dependency, and limited generalization. LLMs show promise but suffer from hallucination and lack structured TRIZ knowledge grounding.

Method: Proposes TRIZ-RAGNER framework that reformulates contradiction mining as semantic-level NER, integrating dense retrieval over TRIZ knowledge base, cross-encoder reranking for context refinement, and structured LLM prompting to extract improving/worsening parameters from patent sentences.

Result: Achieves 85.6% precision, 82.9% recall, and 84.2% F1-score in TRIZ contradiction pair identification on PaTRIZ dataset, outperforming traditional sequence labeling models and LLM baselines. Shows 7.3 percentage point F1 improvement over strongest prompt-enhanced GPT baseline.

Conclusion: Retrieval-augmented TRIZ knowledge grounding effectively reduces semantic noise and improves extraction consistency, confirming the effectiveness of the proposed framework for robust and accurate patent-based contradiction mining.
...
<details>
  <summary>Details</summary>
Motivation: Static datasets for LLM evaluation have limited scalability and fail to capture evolving reasoning capabilities; need dynamic approaches that adapt to model improvements.

Method: Three-agent protocol: teacher generates problems, orchestrator validates them and guards against attacks, student solves problems. Invalid problems are revised; successful solutions trigger generation of more challenging variants. Uses text anomaly detection format requiring cross-sentence inference.

Result: Protocol systematically exposes corner-case reasoning errors that conventional benchmarks miss, enables automatic difficulty scaling as more capable agents are used, and supports evaluation along multiple axes including cross-model pairwise performance.
...
<details>
  <summary>Details</summary>
Motivation: Addresses three key challenges in few-shot text classification: semantic entanglement (confused semantic boundaries), unclear label structure (ambiguous label semantics), and insufficient feature representation (poor discriminative power under low-resource conditions).

Method: 1. Uses pretrained language model for basic text encoding. 2. Introduces structured prompts with multi-dimensional semantic factors integrated via learnable combination mechanism. 3. Constructs structured label embedding matrix with cross-space alignment for text-label semantic consistency. 4. Applies prompt orthogonality constraints and joint optimization to maintain semantic factor independence.

Result: Experimental results show the framework effectively alleviates semantic conflicts and label ambiguity, significantly improves performance on accuracy, precision, recall, and AUC metrics, and demonstrates strong cross-task applicability and robustness.

Conclusion: The structured prompt optimization framework provides transparent and controllable guidance for classification decisions in few-shot text classification, enhancing semantic understanding and task adaptation under low-resource conditions while maintaining stability across different experimental settings.
...
<details>
  <summary>Details</summary>
Motivation: There's a substantial gap between the theoretical parallelism of diffusion-based LLMs (which can generate multiple tokens in parallel) and their practical performance, as current dLLMs still use one-token-per-step generation due to quality degradation when decoding multiple masked tokens simultaneously.

Method: DiCo employs a three-phase divide-and-conquer approach: 1) Divide phase identifies seed tokens and expands them into local clusters; 2) Conquer phase performs parallel decoding across different clusters; 3) Finalize phase uses fine-grained compound decoding for remaining masked tokens. The process alternates between Divide and Conquer phases until convergence.
...
Conclusion: DiCo successfully bridges the gap between theoretical parallelism and practical performance in diffusion-based LLMs through its adaptive parallel decoding approach, enabling efficient parallel generation without sacrificing quality.
...
TL;DR: GLUScope is an open-source tool for analyzing neurons in Transformer-based language models with gated activation functions like SwiGLU, addressing the challenge of interpreting four sign combinations of gate and activation values.


<details>
  <summary>Details</summary>
Motivation: Existing interpretability tools focus on older models and don't adequately handle newer models with gated activation functions like SwiGLU, where understanding neuron behavior requires analyzing both gate and activation sign combinations rather than just positive activations.

Method: Developed an open-source tool that analyzes neurons by categorizing their behavior into four sign combinations (gate+/activation+, gate+/activation-, gate-/activation+, gate-/activation-), shows text examples for each combination, and tracks frequency of occurrence for each pattern.

Result: GLUScope successfully analyzes neurons in models with gated activation functions, provides interpretability insights through categorized examples, and demonstrates novel insights about neuron functionality based on sign combinations. A demo is available online.

Conclusion: GLUScope addresses a gap in interpretability research by providing specialized analysis tools for modern Transformer models with gated activation functions, enabling deeper understanding of neuron behavior through sign combination analysis.
...
TL;DR: CLFEC introduces a new task for joint linguistic and factual error correction in Chinese professional writing, addressing the co-occurrence of both error types through systematic evaluation of LLM-based approaches on a multi-domain dataset.


<details>
  <summary>Details</summary>
Motivation: Traditional Chinese text correction separates spelling/grammar from factual errors, but in professional writing these errors frequently co-occur and interact, making unified correction necessary yet challenging for reliable proofreading systems.

Method: Created CLFEC task and constructed a multi-domain Chinese professional writing dataset (current affairs, finance, law, medicine). Systematically studied LLM-based correction paradigms: prompting, retrieval-augmented generation (RAG), and agentic workflows.

Result: Analysis revealed practical challenges: limited generalization of specialized models, need for evidence grounding for factual repair, difficulty with mixed-error paragraphs, and over-correction on clean inputs. Joint correction outperformed decoupled processes, and agentic workflows were effective with suitable backbone models.

Conclusion: The CLFEC dataset and empirical findings provide guidance for building reliable, fully automatic proofreading systems in industrial settings, demonstrating that unified linguistic and factual error correction is both necessary and achievable with appropriate approaches.
...
<details>
  <summary>Details</summary>
Motivation: To investigate the extent to which structural cues (morphosyntax, closed-class words) can constrain lexical meaning in language processing, and to understand LLMs' ability to recover meaning from degraded text.

Method: Created "Jabberwockified" English texts by randomly substituting content words with nonsense strings, then tested LLMs' ability to translate these degraded texts back to conventional English.

Result: LLMs show astonishing ability to recover meaning from severely degraded texts, often producing translations close to the original text, demonstrating superhuman performance in this task.

Conclusion: Structural cues constrain lexical meaning to a much larger degree than imagined, and efficient language processing requires tight integration between syntax, lexical semantics, and world knowledge in both biological and artificial systems.
...
<details>
  <summary>Details</summary>
Motivation: Nepali is a low-resource language written in Devanagari script that remains relatively underexplored in NLP, despite significant advances in transformer-based models for many other languages.

Method: Ten pre-trained BERT variants (mBERT, XLM-R, MuRIL, DevBERT, HindiBERT, IndicBERT, NepBERTa) were fine-tuned and tested on a balanced Nepali dataset containing 25,006 sentences across five conceptual domains. Performance was evaluated using accuracy, weighted precision, recall, F1-score, and AUROC metrics.

Result: Indic models, particularly MuRIL-large, achieved the highest F1-score of 90.60%, outperforming both multilingual and monolingual models. NepBERTa also performed competitively with an F1-score of 88.26%.

Conclusion: The findings establish a robust baseline for future document-level classification and broader Nepali NLP applications, demonstrating that Indic language models are particularly effective for Nepali despite its low-resource status.
...
TL;DR: Researchers created a gold standard dataset of geographic coordinates from 18th-century French encyclopedias, trained transformer models to extract and normalize coordinates, and demonstrated cross-lingual/cross-domain generalization to other historical texts.


<details>
  <summary>Details</summary>
Motivation: Automatically recovering geographic coordinates from historical texts is challenging due to varied expression formats and precision levels in early modern documents. There's a need for improved methods to extract and normalize coordinates from digitized historical texts.

Method: Created a gold standard dataset from Diderot and d'Alembert's Encyclopedie (15,278 geographical entries, 4,798 with coordinates). Trained transformer-based models using a two-step pipeline: classifier to identify coordinate-bearing entries, then retrieval model for extraction. Tested encoder-decoder and decoder architectures.

Result: Cross-validation achieved 86% EM score. Out-of-domain testing showed 61% EM on 18th-century French Trevoux dictionary and 77% EM on 19th-century English Encyclopaedia Britannica. Demonstrated cross-lingual and cross-domain generalization.

Conclusion: The gold standard dataset is valuable training data for coordinate extraction from historical texts. The two-step method shows strong cross-lingual and cross-domain generalizability, making it applicable to various early modern digitized texts beyond the original training corpus.
...
TL;DR: The paper introduces HLME, an emotion-enhanced memory evaluation benchmark to assess how well LLM memory systems handle emotional information, finding current systems inadequate across three key emotional memory tasks.


<details>
  <summary>Details</summary>
Motivation: Current memory systems for LLMs address context loss in prolonged interactions, but their effectiveness in processing emotion-related information compared to human cognition remains unclear and needs systematic evaluation.

Method: Created the Human-Like Memory Emotion (HLME) dataset and benchmark that evaluates memory systems across three dimensions: emotional information extraction, emotional memory updating, and emotional memory question answering.

Result: Experimental results show that none of the evaluated mainstream and state-of-the-art memory systems achieve robust performance across all three emotional memory tasks, revealing significant deficiencies.

Conclusion: The findings provide objective evidence of current memory systems' limitations in processing emotional memories and suggest a new research direction for improving emotional memory capabilities in LLMs.
...
TL;DR: gradiend is an open-source Python package implementing the GRADIEND method for learning feature directions from factual-counterfactual MLM and CLM gradients in language models, providing tools for feature analysis, model rewriting, and multi-feature comparison.


<details>
  <summary>Details</summary>
Motivation: To provide a unified, operationalized tool for analyzing and manipulating language model representations by learning feature directions from factual-counterfactual gradients, enabling systematic feature analysis and controlled model editing.

Method: GRADIEND method learns feature directions from factual-counterfactual masked language modeling (MLM) and causal language modeling (CLM) gradients, implemented in an open-source Python package with workflows for data creation, training, evaluation, visualization, persistent model rewriting via controlled weight updates, and multi-feature comparison.

Result: The package successfully demonstrates GRADIEND on an English pronoun paradigm and reproduces prior use cases in a large-scale feature comparison, showing its effectiveness for feature analysis and model manipulation.

Conclusion: gradiend provides a comprehensive, operationalized framework for learning feature directions from language model gradients, enabling systematic feature analysis, visualization, and controlled model rewriting for interpretability and editing applications.
...
TL;DR: YouTube's Spanish automatic captioning system shows dialectal biases, performing better for some Spanish dialects than others, highlighting the need for more inclusive algorithmic technologies.


<details>
  <summary>Details</summary>
Motivation: YouTube offers only one automatic captioning system for Spanish despite significant dialectal variations across 21 Spanish-speaking countries, raising concerns about potential biases against certain dialects in speech recognition technology.
...
Result: Identified systematic disparities in caption quality attributable to specific Spanish dialects, demonstrating that the system performs better for some dialects than others, revealing dialectal bias in the technology.

Conclusion: Algorithmic technologies deployed on digital platforms need to be calibrated to diverse user populations and their linguistic variations, requiring more inclusive design to avoid dialectal discrimination.
...
TL;DR: Reasoning in LLMs doesn't universally improve performance - effectiveness is strongly task-dependent, with binary classification degrading up to -19.9 F1 points while complex emotion recognition gains up to +16.0 points.


<details>
  <summary>Details</summary>
Motivation: To challenge the prevailing narrative that reasoning universally improves LLM performance across language tasks by conducting comprehensive evaluation across diverse reasoning architectures and task complexities.

Method: Comprehensive evaluation of 504 configurations across seven model families (adaptive, conditional, and reinforcement learning-based reasoning architectures) on sentiment analysis datasets with varying granularity (binary, 5-class, and 27-class emotion). Used few-shot prompting, distilled reasoning variants, and Pareto frontier analysis for efficiency-performance trade-offs.

Result: 1) Reasoning effectiveness is task-complexity dependent - binary classification degrades up to -19.9 F1 points, while 27-class emotion recognition gains up to +16.0 points. 2) Distilled reasoning variants underperform base models by 3-18 points on simpler tasks. 3) Few-shot learning improves over zero-shot in most cases. 4) Base models dominate efficiency-performance trade-offs, with reasoning justified only for complex emotion recognition despite 2.1x-54x computational overhead.

Conclusion: Reasoning in LLMs is not universally beneficial - its effectiveness depends strongly on task complexity. Simple tasks degrade due to systematic over-deliberation, while complex tasks benefit. Base models remain optimal for efficiency-performance trade-offs, with reasoning only justified for high-complexity tasks despite significant computational costs.
...
TL;DR: Preference packing is a resource-efficient training optimization method that reduces attention operations and KV cache memory usage for preference learning tasks by packing multiple responses to the same input prompt, achieving at least 37% training time reduction and up to 3.22x speedup when combined with existing techniques.


<details>
  <summary>Details</summary>
Motivation: As large language models grow in size, resource-efficient training optimization becomes increasingly important. Batch packing is commonly used in pre-training and supervised fine-tuning, but there's a need for similar efficiency improvements in preference learning tasks like reward modeling and Direct Preference Optimization (DPO) that involve multiple responses to the same input prompt.

Method: Preference packing enhances resource efficiency by reducing attention operations for duplicate input prompts and decreasing KV cache memory usage. The method packs data with different responses for the same input prompt, which is particularly useful for preference learning techniques. It can be applied alongside existing optimization techniques like batch sorting.

Result: Experiments on text-only and image-included datasets showed at least 37% reduction in training time. When combined with existing optimization techniques such as batch sorting, the method achieved a 3.22x speedup.

Conclusion: Preference packing is an effective resource-efficient training optimization method for preference learning tasks that significantly reduces training time and computational resources while being compatible with existing optimization techniques.
...
TL;DR: ARGUS framework studies how narratives impact persuasion in online arguments, using annotated ChangeMyView corpus and LLMs to analyze six narrative features' effects on persuasion success.


<details>
  <summary>Details</summary>
Motivation: Despite narratives being seen as powerful persuasive tools, their specific role in online, unstructured argumentation remains underexplored. The paper aims to address this gap by systematically studying how narrative features influence persuasion in argumentative discourse.

Method: ARGUS framework introduces a new ChangeMyView corpus annotated for story presence and six key narrative features, integrating insights from two established theoretical frameworks. It leverages both encoder-based classifiers and zero-shot large language models (LLMs) to identify stories and narrative features at scale, examining how different narrative dimensions influence persuasion success.

Result: The paper presents ARGUS as a comprehensive framework for analyzing narrative impact on persuasion, though specific quantitative results about which narrative features matter most are not detailed in the abstract (likely presented in the full paper).

Conclusion: ARGUS provides a systematic approach to studying narrative persuasion in online argumentation, enabling large-scale analysis of how specific narrative features contribute to persuasion success, addressing a significant gap in understanding narrative effects in unstructured digital discourse.
...
TL;DR: First systematic human evaluation of LLM translation for Ancient Greek technical prose shows high quality on translated texts (95.2/100 MQM) and moderate quality on untranslated pharmacological texts (79.9/100), with terminology rarity strongly predicting translation failure.


<details>
  <summary>Details</summary>
Motivation: To systematically evaluate LLM machine translation quality for Ancient Greek technical prose using both automated metrics and expert human evaluation, particularly focusing on texts with varying translation availability and terminological density.

Method: Evaluated translations by three commercial LLMs (Claude, Gemini, ChatGPT) of 20 paragraph-length passages from Galen's works. Used both standard automated metrics (BLEU, chrF++, METEOR, ROUGE-L, BERTScore, COMET, BLEURT) and expert human evaluation via modified Multidimensional Quality Metrics framework applied by domain specialists to all 60 translations.

Result: On previously translated expository text (On Mixtures), LLMs achieved high translation quality (mean MQM score 95.2/100), approaching expert level. On untranslated pharmacological text (On the Composition of Drugs), aggregate quality was lower (79.9/100) with high variance driven by extreme terminological density. Terminology rarity (corpus frequency) emerged as strong predictor of translation failure (r = -.97). Automated metrics showed moderate correlation with human judgment on text with wide quality spread but failed to discriminate among high-quality translations.

Conclusion: LLMs can produce high-quality translations of Ancient Greek technical prose, especially for texts with existing translations, but struggle with rare terminology in specialized domains. Terminology rarity strongly predicts translation failure. Automated metrics have limited utility for discriminating high-quality translations, highlighting the need for human evaluation in low-resource ancient language contexts.
...
TL;DR: CoME is a novel mobile agent architecture with four specialized experts for different reasoning stages, using output-oriented activation and progressive training to achieve balanced hybrid-capabilities reasoning.


<details>
  <summary>Details</summary>
Motivation: Existing mobile agents struggle to achieve both decoupled enhancement and balanced integration of hybrid capabilities needed for autonomous execution (screen summary, subtask planning, action decision, action function).

Method: CoME architecture with four distinct experts aligned to specific reasoning stages, using output-oriented activation. Progressive training strategy includes Expert-FT (decoupling/enhancement), Router-FT (expert alignment), CoT-FT (collaboration optimization). InfoGain-Driven DPO uses information gain to evaluate intermediate steps and mitigate error propagation.

Result: CoME outperforms dense mobile agents and MoE methods on both AITZ and AMEX datasets, demonstrating superior hybrid-capabilities reasoning performance.

Conclusion: CoME successfully addresses the challenges of decoupled enhancement and balanced integration of hybrid capabilities in mobile agents through specialized experts, progressive training, and information gain-driven optimization.
...
TL;DR: ArgLLM-App is a web-based system implementing argumentative LLM agents for binary decision-making tasks, featuring visualization of explanations and human interaction for contesting reasoning errors.


<details>
  <summary>Details</summary>
Motivation: To create a practical system that makes LLM-based decisions explainable and contestable by humans, addressing the need for transparency and accountability in AI decision-making.

Method: Developed a modular web-based system implementing ArgLLM-empowered agents for binary tasks, supporting visualization of explanations, human interaction for contesting mistakes, and integration with trusted external sources.

Result: Created ArgLLM-App, a publicly available system at https://argllm.app with video demonstration, enabling users to visualize AI reasoning and contest any identified mistakes in the system's decision-making process.

Conclusion: ArgLLM-App provides a practical implementation of argumentative LLMs for transparent decision-making, offering both explanation visualization and human contestation capabilities to enhance trust and accountability in AI systems.
...
<details>
  <summary>Details</summary>
Motivation: Small language models are deployed in high-volume, low-latency settings where efficiency is crucial, but they lack the acceleration techniques available to larger models. There's a need for task-specific optimization methods that can improve inference efficiency while maintaining performance.

Method: TASC framework includes two approaches: 1) TASC-ft: During fine-tuning, iteratively enriches tokenizer vocabulary with high-frequency output n-grams, then fine-tunes model to use expanded vocabulary. 2) TASC-spec: Inference-time speculative decoding method that constructs lightweight n-gram draft model from task's output corpus, mixing task and context n-gram information without additional training.

Result: The methods demonstrate consistent improvements in inference efficiency across multiple low output-variability generation tasks while maintaining task performance. TASC-spec avoids additional training and bypasses draft-target vocabulary alignment constraints.

Conclusion: TASC provides effective task-adaptive sequence compression for SLM acceleration through complementary fine-tuning and inference-time methods, offering practical efficiency gains for high-volume, low-latency applications without compromising task performance.
...
TL;DR: Language models struggle with multi-turn collaborative conversations despite having substantial headroom for improvement, performing worse than non-interactive baselines in games requiring communication about private information.


<details>
  <summary>Details</summary>
Motivation: To evaluate language models' capabilities in multi-turn collaborative interactions, particularly their ability to effectively communicate about private information - a defining feature of real-world communication that current models may lack.

Method: Developed MT-PingEval, a scalable evaluation methodology using collaborative games requiring private information communication. Used interactive scaling analysis where fixed token budget is divided over variable turns, comparing interactive vs. non-interactive baselines. Analyzed linguistic features including sycophancy, information density, and discourse coherence.

Result: Language models fail to leverage interactive collaboration to outperform non-interactive baselines (where one agent summarizes information and the other acts immediately), despite substantial headroom. Humans achieve comparable task success with superior token efficiency through more coherent dialogues. No single linguistic factor explains models' collaborative weaknesses.

Conclusion: State-of-the-art language models still have significant weaknesses in planning and executing multi-turn collaborative conversations. The MT-PingEval framework provides a tool for driving improvements in models' ability to proactively manage private information during communication.
...
TL;DR: Training reasoning models to follow instructions in reasoning traces improves privacy preservation but trades off task utility.


<details>
  <summary>Details</summary>
Motivation: AI agents using reasoning models risk leaking sensitive user data through uncontrolled reasoning traces; improving instruction-following in reasoning could enhance privacy.

Method: Fine-tune models on instruction-following dataset with reasoning trace restrictions; use separate LoRA adapters to decouple reasoning and answer generation.

Result: Achieved up to 20.9-point improvement in instruction-following and 51.9 percentage-point gain on privacy benchmarks across 6 models (1.7B-14B parameters), though with task utility trade-offs.

Conclusion: Improving instruction-following in reasoning models significantly enhances privacy, offering a promising direction for privacy-aware AI agents despite utility trade-offs.
...
TL;DR: Removing prior assistant responses from LLM conversation history often doesn't hurt quality while reducing context length up to 10x, revealing many prompts are self-contained and assistant history can cause context pollution.


<details>
  <summary>Details</summary>
Motivation: To challenge the standard design choice of including LLMs' own prior responses in conversation history and investigate whether models truly benefit from conditioning on their own past outputs in multi-turn interactions.

Method: Compared standard full-context prompting with user-turn-only prompting (omitting all previous assistant responses) across three open reasoning models and one state-of-the-art model using in-the-wild multi-turn conversations. Analyzed prompt self-containment, designed context-filtering approach for selective omission.

Result: Removing prior assistant responses doesn't affect quality on many turns (36.4% prompts are self-contained), reduces context length up to 10x. User-turn-only prompting sometimes outperforms full context due to context pollution where models over-condition on previous responses, causing error propagation.

Conclusion: Selectively omitting assistant history can improve response quality while reducing memory consumption, challenging the necessity of always including LLMs' own prior responses in conversation context.
...
<details>
  <summary>Details</summary>
Motivation: Existing datasets lack realistic, human-like user queries for evaluating MCP servers, containing tool descriptions but failing to represent how different users portray requests, leading to poor generalization and inflated benchmark reliability.

Method: Developed on MCP Zero dataset, generating diverse user queries specifically matched to 2800 tools across 308 MCP servers, with each tool paired with multiple unique user personas capturing varying levels of user intent from precise task requests to ambiguous exploratory commands.

Result: Created the first large-scale MCP dataset featuring diverse, high-quality user queries that reflect the complexity of real-world interaction patterns, addressing the critical gap in evaluating tool usage and ecosystems of MCP servers.

Conclusion: This dataset provides a more realistic benchmark for evaluating MCP server tool usage by capturing diverse user personas and interaction patterns, enabling better assessment of generalization and reliability in real-world applications.
...
TL;DR: An agentic LLM-RAG system automates adverse media screening for AML/KYC compliance, reducing false positives and manual review by computing Adverse Media Index scores through web search and document processing.


<details>
  <summary>Details</summary>
Motivation: Traditional keyword-based adverse media screening generates high false-positive rates and requires extensive manual review, creating inefficiencies in AML/KYC compliance processes for financial institutions.

Method: Multi-step agentic system using LLMs with RAG: LLM agent searches the web, retrieves and processes relevant documents, and computes an Adverse Media Index (AMI) score for each subject.

Result: System evaluated with multiple LLM backends on dataset including PEPs, regulatory watchlist persons, sanctioned persons from OpenSanctions, and clean names; demonstrates ability to distinguish between high-risk and low-risk individuals.

Conclusion: The LLM-RAG agentic system provides an effective automated solution for adverse media screening, improving accuracy and efficiency over traditional keyword-based approaches in financial compliance.
...
TL;DR: CTFIDU+ algorithm enables complete identification of counterfactual queries from physically realizable Layer 3 distributions, establishing fundamental limits to exact causal inference and providing analytic bounds for non-identifiable quantities.


<details>
  <summary>Details</summary>
Motivation: Previous counterfactual identification research assumed impossibility of obtaining Layer 3 data, but recent work showed some counterfactual distributions can be directly estimated experimentally. This raises the question of what additional counterfactual quantities become identifiable with access to physically realizable Layer 3 data.
...
Result: CTFIDU+ is complete for identifying counterfactual queries from Layer 3 distributions, establishing fundamental limits to exact causal inference in non-parametric setting. Simulations show counterfactual data helps tighten bounds for non-identifiable quantities in practice.

Conclusion: The work characterizes what counterfactuals can be identified given access to physically realizable Layer 3 data, establishes fundamental limits of causal inference, and provides practical bounds for quantities that remain non-identifiable, advancing the theoretical foundations of counterfactual reasoning.
...
TL;DR: A framework for planning under distribution shifts using causal POMDPs that maintains tractability via piecewise linear convex value functions.


<details>
  <summary>Details</summary>
Motivation: Planning in real-world environments faces challenges from distribution shifts where previously learned strategies fail due to changes in state distributions or environment dynamics. Models obtained under one set of conditions may become invalid when distributions shift.

Method: Proposes a theoretical framework using Partially Observable Markov Decision Processes (POMDPs) formulated with causal knowledge. Represents distribution shifts as interventions on this causal POMDP, enabling evaluation of plans under hypothesized changes and active identification of altered environment components. Maintains and updates belief over both latent state and underlying domain.

Result: Proves that the value function remains piecewise linear and convex (PWLC) in the augmented belief space. This preservation of PWLC maintains tractability of planning via α-vector-based POMDP methods even under distribution shifts.

Conclusion: The framework enables robust planning under distribution shifts by leveraging causal knowledge and maintaining computational tractability through preservation of PWLC properties in augmented belief spaces.
...
TL;DR: RL-CMSA: A hybrid reinforcement learning approach for min-max mTSP that combines probabilistic clustering with learned q-values, set-covering MILP, and local search to balance workload across multiple salesmen.


<details>
  <summary>Details</summary>
Motivation: The min-max mTSP aims to minimize the longest tour (workload balance) across multiple salesmen, which is important for practical applications like delivery routing where fairness and efficiency matter. Existing methods need improvement, especially for larger instances.

Method: Hybrid approach with four components: 1) Construct diverse solutions using probabilistic clustering guided by learned pairwise q-values, 2) Merge routes into a compact pool, 3) Solve restricted set-covering MILP, 4) Refine solutions via inter-route remove, shift, and swap moves. Q-values are updated based on city-pair co-occurrences in high-quality solutions, while the pool is adapted through ageing and pruning.

Result: RL-CMSA consistently finds (near-)best solutions and outperforms a state-of-the-art hybrid genetic algorithm under comparable time limits, especially as instance size and number of salesmen increase. Tested on random and TSPLIB instances.

Conclusion: The combination of exact optimization (set-covering MILP) and reinforcement-guided construction effectively balances exploration and exploitation for min-max mTSP. The method demonstrates strong performance scalability and represents a promising hybrid approach for combinatorial optimization problems.
...
TL;DR: SleepLM is a sleep-language foundation model that bridges natural language with multimodal polysomnography data, enabling language-based sleep analysis, querying, and interpretation beyond traditional closed-label systems.


<details>
  <summary>Details</summary>
Motivation: Traditional sleep analysis systems operate in closed label spaces with predefined stages/events, failing to describe, query, or generalize to novel sleep phenomena. There's a need for more flexible, language-grounded representations of sleep physiology.

Method: Developed a multilevel sleep caption generation pipeline to create the first large-scale sleep-text dataset (100K+ hours from 10,000+ individuals). Used unified pretraining combining contrastive alignment, caption generation, and signal reconstruction to capture physiological fidelity and cross-modal interactions.
...
Conclusion: SleepLM successfully bridges natural language with sleep physiology, enabling more flexible and interpretable sleep analysis. The model demonstrates strong performance on real-world tasks and novel capabilities for sleep understanding, with all code and data to be open-sourced.
...
TL;DR: MMKG-RDS is a flexible reasoning data synthesis framework using multimodal knowledge graphs to address limitations in long-tail knowledge coverage, effectiveness verification, and interpretability in existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing methods for synthesizing training data have limitations in long-tail knowledge coverage, effectiveness verification, and interpretability. Knowledge-graph-based approaches still lack functionality, granularity, customizability, and proper evaluation methods.

Method: Proposes MMKG-RDS framework leveraging multimodal knowledge graphs with fine-grained knowledge extraction, customizable path sampling, and multidimensional data quality scoring. Validated using MMKG-RDS-Bench dataset covering 5 domains, 17 task types, and 14,950 samples.

Result: Fine-tuning Qwen3 models (0.6B/8B/32B) on synthesized samples improves reasoning accuracy by 9.2%. Framework generates challenging data for tasks involving tables and formulas, useful for complex benchmark construction.

Conclusion: MMKG-RDS provides an effective framework for synthesizing high-quality reasoning training data using multimodal knowledge graphs, addressing key limitations of existing methods and demonstrating practical improvements in model reasoning capabilities.
...
TL;DR: The paper critiques the concept of Artificial General Intelligence (AGI) as flawed and proposes Superhuman Adaptable Intelligence (SAI) as a better framework - intelligence that can learn to exceed humans at important tasks and fill human skill gaps through specialization rather than striving for human-like generality.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the confusion and flawed thinking around AGI definitions, which are often incoherent and anthropocentric. The authors argue that the pursuit of human-like generality is misguided, and that AI should instead focus on specialized superhuman capabilities that can address human limitations.

Method: The method involves critical analysis of existing AGI definitions, examining whether they are plausible, useful, and truly general. The authors then propose an alternative framework - Superhuman Adaptable Intelligence (SAI) - defined as intelligence that can learn to exceed humans at important tasks and fill human skill gaps, emphasizing specialization over generality.

Result: The paper establishes that current AGI conceptions are flawed and proposes SAI as a superior conceptual framework. SAI provides clearer guidance for AI development by focusing on specialization and superhuman performance rather than anthropomorphic generality.

Conclusion: The conclusion is that AI should abandon the flawed AGI concept and instead embrace specialization through the SAI framework, which offers more practical guidance for developing AI that can exceed human capabilities where it matters most and address human limitations.
...
TL;DR: PseudoAct introduces a pseudocode synthesis framework for LLM agents that replaces reactive decision-making with structured planning, enabling explicit control flow and reducing redundancy in complex long-horizon tasks.
...
Method: PseudoAct synthesizes structured pseudocode plans that decompose tasks into subtasks with explicit control flow encoding (sequencing, conditionals, loops, parallel composition). Actions are executed by following this global plan, making decision logic explicit and temporally coherent.

Result: Significantly outperforms existing reactive agent approaches with 20.93% absolute gain in success rate on FEVER and sets new state-of-the-art on HotpotQA. Reduces redundant actions, prevents infinite loops, and avoids uninformative alternative exploration.

Conclusion: PseudoAct enables consistent and efficient long-horizon decision-making in LLM agents through pseudocode synthesis, addressing limitations of reactive approaches by providing structured planning with explicit control flow.
...
TL;DR: ODAR-Expert is an adaptive routing framework that optimizes accuracy-efficiency trade-offs by dynamically allocating compute resources between fast and slow LLM agents based on query difficulty, using free-energy-based decision-making instead of uniform brute-force sampling.


<details>
  <summary>Details</summary>
Motivation: Current LLM reasoning approaches rely on costly uniform brute-force sampling (like best-of-N or self-consistency) that suffers from diminishing returns, overthinking, and poor attribution. There's a need to shift from simply scaling test-time compute to adaptive resource allocation that optimizes the accuracy-efficiency trade-off.

Method: ODAR-Expert uses a difficulty estimator based on amortized active inference to dynamically route queries between a heuristic Fast Agent and a deliberative Slow Agent. It introduces a free-energy-principled, risk-sensitive fusion mechanism that selects answers by minimizing a variational free energy objective, balancing log-likelihood with epistemic uncertainty (varentropy) as an alternative to ad hoc voting.
...
TL;DR: CHIEF is a hierarchical causal analysis framework for LLM-powered multi-agent systems that transforms chaotic execution logs into structured causal graphs and uses oracle-guided backtracking with progressive causal screening to accurately identify root causes of failures.
...
Method: CHIEF transforms chaotic trajectories into structured hierarchical causal graphs, employs hierarchical oracle-guided backtracking to efficiently prune search space via synthesized virtual oracles, and implements counterfactual attribution through progressive causal screening to distinguish true root causes from propagated symptoms.
...
Conclusion: CHIEF provides a novel hierarchical causal analysis framework that effectively addresses the limitations of existing flat-sequence approaches to failure attribution in LLM-powered multi-agent systems, offering improved observability and precise responsibility assignment.
...
TL;DR: ProductResearch is a multi-agent framework that generates synthetic training trajectories for e-commerce shopping agents, enabling LLMs to perform comprehensive product research through fine-tuning on distilled multi-agent interactions.
...
Method: A multi-agent framework with User Agent (infers shopping intents), Supervisor Agent (orchestrates collaboration), and Research Agent generates synthetic tool-use trajectories. These are filtered, distilled through reflective internalization into single-role training examples for LLM fine-tuning.

Result: A compact MoE model fine-tuned on the synthetic data shows substantial improvements over base models in response comprehensiveness, research depth, and user-perceived utility, approaching performance of frontier proprietary deep research systems.

Conclusion: Multi-agent synthetic trajectory training is an effective and scalable paradigm for enhancing LLM-based shopping assistance, enabling robust e-commerce conversational agents through high-fidelity training data generation.
...
TL;DR: The Auton Agentic AI Framework bridges the architectural mismatch between stochastic LLM outputs and deterministic backend systems through a principled architecture with cognitive blueprints, runtime engines, and formal safety mechanisms.


<details>
  <summary>Details</summary>
Motivation: The transition from Generative AI to Agentic AI exposes a fundamental architectural mismatch: LLMs produce stochastic, unstructured outputs while backend infrastructure requires deterministic, schema-conformant inputs. Current approaches lack standardization for creating, executing, and governing autonomous agent systems.

Method: The framework introduces a strict separation between Cognitive Blueprint (declarative, language-agnostic specification) and Runtime Engine (platform-specific execution substrate). It formalizes agent execution as an augmented POMDP with latent reasoning, implements hierarchical memory consolidation inspired by biological episodic memory, uses constraint manifolds for safety via policy projection, and includes a three-level self-evolution framework with runtime optimizations like parallel graph execution and speculative inference.

Result: The architecture enables cross-language portability, formal auditability, and modular tool integration via Model Context Protocol (MCP). Runtime optimizations reduce end-to-end latency for multi-step agent workflows while maintaining safety through formal constraint enforcement rather than post-hoc filtering.

Conclusion: The Auton Agentic AI Framework provides a principled, standardized architecture for autonomous agent systems that bridges the stochastic-deterministic gap, enabling scalable, portable, and governable Agentic AI with formal safety guarantees and performance optimizations.
...
TL;DR: MERaLiON2-Omni (Alpha) is a 10B-parameter multilingual MLLM for Southeast Asia that uses progressive training to decouple perception and reasoning, revealing an efficiency-stability paradox where reasoning boosts abstract tasks but destabilizes sensory processing.


<details>
  <summary>Details</summary>
Motivation: Current MLLMs lack robust sensory grounding combined with complex reasoning, especially for underrepresented regions like Southeast Asia, where local cultural and linguistic nuances are often overlooked.

Method: Progressive training pipeline: 1) Build Perception Backbone by aligning region-specific audio-visual cues via orthogonal modality adaptation; 2) Inject reasoning via Generate-Judge-Refine pipeline using Super-LLM to filter hallucinations and synthesize silver data for multimodal Chain-of-Thought.

Result: Evaluation on SEA-Omni Benchmark reveals Efficiency-Stability Paradox: reasoning amplifies abstract task performance (math, instruction-following) but introduces instability in sensory processing (Temporal Drift in audio, Visual Over-interpretation).

Conclusion: The research demonstrates a novel approach to integrating perception and reasoning in MLLMs for underrepresented regions, while identifying critical trade-offs that need to be addressed for stable multimodal reasoning systems.
...
TL;DR: RD-MLDG is a novel domain generalization framework that leverages multimodal LLMs' reasoning capabilities with reasoning chains to improve robustness under domain shift, addressing challenges in optimizing complex reasoning sequences through multi-task cross-training and self-aligned reasoning regularization.


<details>
  <summary>Details</summary>
Motivation: Most domain generalization methods focus on visual feature invariance, but the authors explore leveraging multimodal LLMs' reasoning capabilities to construct reasoning chains for more robust predictions under domain shift, recognizing that reasoning could provide complementary signals for out-of-domain generalization.

Method: The paper introduces RD-MLDG with two key components: (1) MTCT (Multi-Task Cross-Training) adds a direct classification pathway to guide reasoning supervision, and (2) SARR (Self-Aligned Reasoning Regularization) preserves semantic richness of reasoning chains while mitigating reasoning-pattern mismatches through iterative self-labeling. The approach is evaluated on DomainBed-Reasoning, a newly constructed extension of DomainBed with reasoning chains.

Result: Experiments on standard DomainBed datasets (PACS, VLCS, OfficeHome, TerraInc) demonstrate that RD-MLDG achieves state-of-the-art performance, showing that reasoning can serve as a promising complementary signal for robust out-of-domain generalization.

Conclusion: Reasoning chains from multimodal LLMs provide valuable complementary signals for domain generalization, and the proposed RD-MLDG framework effectively addresses the optimization challenges of using reasoning supervision while preserving semantic richness, leading to improved robustness under domain shift.
...
TL;DR: EMO-R3 is a reflective reinforcement learning framework that enhances multimodal LLMs' emotional reasoning through structured thinking and reflective rewards, improving interpretability and performance on emotional understanding tasks.


<details>
  <summary>Details</summary>
Motivation: Current MLLMs struggle with complex, subjective human emotions. Supervised fine-tuning has limited generalization and poor interpretability, while existing RL methods like Group Relative Policy Optimization fail to align with emotional cognition characteristics.

Method: Proposes EMO-R3 framework with two key components: 1) Structured Emotional Thinking for step-by-step, interpretable emotional reasoning, and 2) Reflective Emotional Reward that enables models to re-evaluate reasoning based on visual-text consistency and emotional coherence.
...
Conclusion: EMO-R3 effectively addresses limitations of existing approaches by combining structured reasoning with reflective reinforcement learning, advancing MLLMs' capability for complex emotional understanding and interpretation.
...
TL;DR: RF-Agent uses LLMs as language agents with Monte Carlo Tree Search to design reward functions for low-level control tasks, improving historical feedback utilization and search efficiency.


<details>
  <summary>Details</summary>
Motivation: Existing methods for reward function design in low-level control tasks rely heavily on expert experience or use LLMs with inefficient greedy/evolutionary algorithms that poorly utilize historical feedback and have limited effectiveness in complex tasks.

Method: RF-Agent treats LLMs as language agents and frames reward function design as a sequential decision-making process, integrating Monte Carlo Tree Search (MCTS) to manage the reward design and optimization process, leveraging LLMs' multi-stage contextual reasoning ability.

Result: Outstanding experimental results in 17 diverse low-level control tasks demonstrate the effectiveness of the method, showing improved performance over existing approaches.

Conclusion: The RF-Agent framework successfully addresses limitations of previous reward function design methods by better utilizing historical information and improving search efficiency through MCTS-enhanced LLM reasoning.
...
TL;DR: PAPER proposes using a pessimistic auxiliary policy to sample reliable actions in offline RL, reducing approximation errors and overestimation by maximizing Q-function lower confidence bounds.


<details>
  <summary>Details</summary>
Motivation: Offline RL faces challenges with out-of-distribution actions during learning, which introduce approximation errors that accumulate and cause overestimation. Existing methods struggle with sampling high-value actions that may have high uncertainty, leading to error propagation.

Method: Constructs a pessimistic auxiliary policy that maximizes the lower confidence bound of the Q-function. This policy samples actions with relatively high value but low uncertainty near the learned policy, avoiding high-error actions. The auxiliary strategy reduces approximation errors introduced during action sampling.

Result: Extensive experiments on offline RL benchmarks show the pessimistic auxiliary strategy effectively improves the performance of various offline RL approaches by alleviating error accumulation and reducing overestimation.

Conclusion: The proposed pessimistic auxiliary policy provides a reliable action sampling mechanism that mitigates approximation errors in offline RL, leading to improved learning efficacy across different offline RL methods.
...
Conclusion: The proposed SCR method with counterfactual state construction effectively addresses reward-transition mismatch in RL critic training, enabling stable learning and significant performance improvements in portfolio rebalancing under market regime shifts and stress events.
...
TL;DR: CIRCLE is a six-stage lifecycle framework that bridges the gap between AI model metrics and real-world outcomes by formalizing stakeholder concerns into measurable signals through field testing and longitudinal studies.


<details>
  <summary>Details</summary>
Motivation: Existing frameworks like MLOps focus on system stability while benchmarks measure abstract capabilities, leaving decision-makers without systematic evidence about AI behavior under real-world user variability and constraints. There's a need to connect stakeholder concerns outside the AI stack to measurable outcomes.

Method: CIRCLE operationalizes the Validation phase of TEVV (Test, Evaluation, Verification, and Validation) through a six-stage lifecycle framework. It formalizes translation of stakeholder concerns into measurable signals, integrating field testing, red teaming, and longitudinal studies into a coordinated pipeline. Unlike participatory design or retrospective algorithmic audits, CIRCLE provides a structured, prospective protocol for linking context-sensitive qualitative insights to scalable quantitative metrics.

Result: CIRCLE produces systematic knowledge that is comparable across sites yet sensitive to local context. It enables evidence-based governance based on materialized downstream effects rather than theoretical capabilities.

Conclusion: CIRCLE bridges the reality gap between model-centric metrics and AI's materialized outcomes by providing a structured framework for validating AI systems in real-world contexts, enabling governance based on actual performance rather than theoretical capabilities.
...
TL;DR: First Turing test for speech-to-speech systems reveals none pass human-likeness, identifies paralinguistic features as key bottleneck, proposes interpretable evaluation model.


<details>
  <summary>Details</summary>
Motivation: To determine whether modern speech-to-speech conversational systems can truly converse like humans by conducting the first comprehensive Turing test for S2S systems and moving beyond binary outcomes to provide diagnostic insights.

Method: Conducted first Turing test for S2S systems with 2,968 human judgments on dialogues between 9 state-of-the-art S2S systems and 28 human participants; developed fine-grained taxonomy of 18 human-likeness dimensions and crowd-annotated dialogues; proposed interpretable model leveraging fine-grained ratings for accurate human-vs-machine discrimination.

Result: No existing evaluated S2S system passes the Turing test; bottleneck is not semantic understanding but paralinguistic features, emotional expressivity, and conversational persona; off-the-shelf AI models perform unreliably as Turing test judges; proposed interpretable model delivers accurate and transparent discrimination.

Conclusion: Establishes first human-likeness evaluation framework for S2S systems, reveals significant gap in current systems' human-likeness, provides diagnostic insights beyond binary outcomes, and offers tools for systematic improvement toward human-like conversational AI.
...
TL;DR: Novel bi-level optimization framework using RL for network partitioning and multi-objective VRP for winter road maintenance, validated on UK strategic road networks with significant operational improvements.


<details>
  <summary>Details</summary>
Motivation: Existing winter road maintenance methods struggle with large-scale routing problems and rely heavily on human decision-making, lacking efficiency and scalability for real-world strategic road networks.

Method: Bi-level optimization framework: upper level uses reinforcement learning for strategic network partitioning and resource allocation from multiple depots; lower level solves multi-objective vehicle routing problem within clusters to minimize maximum travel time and total carbon emissions.

Result: Significant improvements including balanced workloads, reduced maximum travel times below targeted two-hour threshold, lower emissions, and substantial cost savings when validated on UK strategic road networks (M25, M6, A1) with interconnected local networks.

Conclusion: Advanced AI-driven bi-level optimization can directly enhance operational decision-making in real-world transportation and logistics, providing scalable solution for winter road maintenance challenges.
...
TL;DR: AAP proposes building AI as resource-bounded agents driven by curiosity-as-learning-progress, treating AI as part of an extended human-tool system to enhance capabilities while reducing interface friction.


<details>
  <summary>Details</summary>
Motivation: The motivation is to develop AI systems that are reality-embedded and resource-bounded, functioning as part of extended human-tool systems to increase sensing, understanding, and actuation capabilities while minimizing friction between people, tools, and environments.

Method: The method unifies predictive compression, intrinsic motivation, empowerment and control, interface quality unification, and language/self-communication as selective information bottlenecks. It formulates these as a falsifiable program with explicit costs, staged experiments, and a concrete multimodal tokenized testbed where agents allocate limited budget among observation, action, and deliberation.

Result: The paper presents a conceptual and experimental framework that connects intrinsic motivation, information theory, thermodynamics, bounded rationality, and modern reasoning systems, though specific empirical results are not detailed in the abstract.

Conclusion: AAP provides a comprehensive research agenda for developing AI as resource-bounded agents driven by curiosity-as-learning-progress, offering a framework that bridges multiple theoretical domains and provides concrete experimental testbeds for validation.
...
TL;DR: SCOPE framework uses Process Reward Models to identify first errors in suboptimal rollouts and applies step-wise correction to salvage partially correct trajectories, improving exploration diversity and achieving SOTA results on math reasoning tasks.


<details>
  <summary>Details</summary>
Motivation: Standard RLVR suffers from coarse outcome-based supervision that penalizes largely correct trajectories as heavily as completely erroneous ones, causing valuable partially correct rollouts to be discarded and leading to premature narrowing of exploration space. Existing methods fail to effectively utilize these partially correct rollouts.

Method: SCOPE (Step-wise Correction for On-Policy Exploration) uses Process Reward Models to pinpoint the first erroneous step in suboptimal rollouts and applies fine-grained, step-wise off-policy rectification to salvage partially correct trajectories, maintaining rollout diversity.

Result: Achieves 13.5% increase in diversity score, sustains broad exploration space, establishes new SOTA with 46.6% average accuracy on math reasoning, and shows robust generalization with 53.4% accuracy on out-of-distribution reasoning tasks.

Conclusion: SCOPE effectively addresses the limitations of standard RLVR by salvaging partially correct trajectories through step-wise correction, maintaining exploration diversity, and achieving superior performance on complex reasoning tasks with strong generalization capabilities.
...
TL;DR: A dynamic benchmark for evaluating LLMs on research-level mathematics using automatically extracted lemmas from arXiv, showing current models achieve only 10-15% accuracy in theorem proving.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks for LLM mathematical capabilities rely on static, hand-curated contest or textbook problems, which don't accurately reflect research-level mathematics. There's a need for benchmarks that directly evaluate models on actual mathematical research.

Method: Developed an automatic pipeline that extracts lemmas from arXiv papers and rewrites them into self-contained statements by making all assumptions and required definitions explicit. This creates an updatable benchmark that can be regularly refreshed with new research problems.

Result: Current state-of-the-art LLMs achieve approximately 10-15% accuracy (pass@1) in theorem proving on this benchmark, indicating a significant performance gap compared to human-level proving capabilities in research contexts.

Conclusion: The proposed benchmark provides a more realistic assessment of LLM capabilities on research mathematics, reveals substantial room for improvement in theorem proving, and enables ongoing evaluation without compromising future assessments through its updatable design.
...
TL;DR: UMPIRE is a training-free uncertainty quantification framework for Multimodal Large Language Models that computes incoherence-adjusted semantic volume of sampled responses, outperforming existing methods across various modalities without external tools.


<details>
  <summary>Details</summary>
Motivation: MLLMs produce plausible but erroneous outputs that hinder reliable deployment. Existing uncertainty metrics have practical constraints: modality-specific designs, reliance on external tools, or computational expense. There's a need for efficient, generalizable uncertainty quantification across modalities.

Method: UMPIRE computes incoherence-adjusted semantic volume of sampled MLLM responses using only internal modality features. It captures both global semantic diversity of samples and local incoherence based on internal model confidence, without training or external tools.
...
Conclusion: UMPIRE provides an effective, training-free uncertainty quantification framework for MLLMs that works efficiently across various input and output modalities without external tools, addressing practical constraints of existing methods and enabling more reliable deployment.
...
TL;DR: A minimal agentic baseline for AI theorem provers that enables systematic comparison across architectures, featuring iterative proof refinement, library search, and context management, achieving competitive performance with simpler design.


<details>
  <summary>Details</summary>
Motivation: To establish a standardized baseline for comparing different AI-based theorem prover architectures, addressing the need for systematic evaluation across diverse approaches and enabling clearer understanding of design choices.

Method: Develops a minimal agentic baseline implementing core features shared by state-of-the-art systems: iterative proof refinement, library search, and context management. Evaluates using qualitatively different benchmarks and compares various popular models and design choices.

Result: Demonstrates competitive performance compared to state-of-the-art approaches while using significantly simpler architecture. Shows consistent advantages of iterative approach over multiple single-shot generations, particularly in sample efficiency and cost effectiveness.

Conclusion: The minimal baseline provides a valuable reference for future research and an accessible prover for the community, highlighting the effectiveness of iterative approaches and the importance of systematic architectural comparisons in AI theorem proving.
...
TL;DR: DARE-bench is a benchmark for evaluating LLMs on multi-step data science tasks with verifiable ground truth, addressing gaps in standardized process-aware evaluation and training data scarcity.
...
Method: Introduced DARE-bench with 6,300 Kaggle-derived tasks featuring verifiable ground truth (not relying on human/model judges). Provides both large-scale training data and evaluation sets covering broad range of tasks with support for agentic tools.

Result: Even highly capable models like gpt-o4-mini struggle, especially in ML modeling tasks. Fine-tuning with DARE-bench training data substantially improves performance: supervised fine-tuning boosts Qwen3-32B accuracy by 1.83x, reinforcement learning boosts Qwen3-4B accuracy by >8x.

Conclusion: DARE-bench serves as both an accurate evaluation benchmark and critical training data source, with significant performance improvements demonstrating its importance for advancing LLM capabilities in data science tasks.
...
TL;DR: The paper proposes Mixed Guidance Graph Optimization (MGGO) methods that optimize both edge weights AND directions in lifelong MAPF, providing strict guidance rather than just soft guidance through weights alone.


<details>
  <summary>Details</summary>
Motivation: Current Guidance Graph Optimization (GGO) methods for lifelong MAPF only optimize edge weights, which provide soft guidance (discouraging but not prohibiting edge traversal). There's a need for strict guidance through edge direction optimization to better control agent movement.

Method: Two MGGO methods: 1) Two-phase approach optimizing edge directions and weights separately; 2) Quality Diversity algorithm optimizing a neural network that generates both edge directions and weights. Also incorporates traffic patterns into GGO for edge-direction-aware guidance.

Result: The paper presents MGGO methods that can optimize both edge weights and directions, providing strict guidance capabilities beyond existing GGO methods that only optimize weights.

Conclusion: Incorporating edge direction optimization into guidance graph methods enables strict guidance in lifelong MAPF, addressing limitations of weight-only optimization and potentially improving agent coordination and system performance.
...
TL;DR: HMASP is a hierarchical multi-agent system that enables end-to-end LLM agentic payment workflows, addressing the gap in current LLM agents' inability to handle payment tasks.


<details>
  <summary>Details</summary>
Motivation: Current LLM agents (like OpenAI's Operator and Claude's Computer Use) can automate workflows but cannot handle payment tasks, and existing solutions struggle with implementing end-to-end agentic payment workflows.

Method: HMASP uses a modular hierarchical architecture with four agent levels: Conversational Payment Agent (entry point), Supervisor agents, Routing agents, and Process summary agent. It employs shared state variables, decoupled message states, and structured handoff protocols for coordination.

Result: Experimental results demonstrate the feasibility of HMASP for implementing end-to-end agentic payment workflows, making it the first LLM-based multi-agent system to achieve this capability.

Conclusion: HMASP successfully addresses the payment workflow gap in LLM agents and lays a foundation for extending agentic capabilities into the payment domain.
...
TL;DR: Multi-agent system for supply networks where factory agents can strategically share information (truth, lies, or mixed strategies) with downstream agents, showing that data sharing boosts performance, especially with cooperative reward shaping.


<details>
  <summary>Details</summary>
Motivation: Current multi-agent models for supply networks assume full observability through shared policies, which is unrealistic due to data privacy concerns. Alternative Hidden-Markov Process approaches are challenging to solve, creating a need for practical information-sharing mechanisms.

Method: Proposed a multi-agent system where factory agents can strategically share information downstream with four options: no sharing, lying, telling truth, or mixed strategies. Combined with cooperative reward shaping to align incentives.

Result: Data sharing boosts performance, especially with cooperative reward shaping. In high demand: limited strategy changes, lying benefits factory slightly for system improvement. In low demand: truth-telling benefits all actors significantly.

Conclusion: Strategic information sharing in multi-agent supply networks can improve system performance, with truth-telling being most beneficial in low-demand scenarios and lying providing marginal gains in high-demand scenarios when combined with cooperative reward mechanisms.
...
TL;DR: QD-MAPPER uses Quality Diversity algorithms with Neural Cellular Automata to automatically generate diverse maps for comprehensive evaluation of Multi-Agent Path Finding algorithms, enabling fair comparisons and algorithm design insights.


<details>
  <summary>Details</summary>
Motivation: Current MAPF algorithm evaluation relies on limited human-designed maps, which may cause overfitting and fail to cover all scenarios. Systematic evaluation on diverse maps is needed for fair comparisons and algorithm improvement.
...
Result: Enables evaluation of various MAPF algorithm types (search-based, priority-based, rule-based, learning-based) through single-algorithm experiments and algorithm comparisons, identifying performance patterns and disparities in runtime/success rates.

Conclusion: QD-MAPPER provides a systematic framework for comprehensive MAPF algorithm evaluation, enabling fair comparisons, algorithm selection guidance, and design insights through automated generation of diverse test maps.
